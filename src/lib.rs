//! ONDEWO NLU (Natural Language Understanding) gRPC client.
//!
//! Everything under [`api`] is GENERATED from the protos of the `ondewo-nlu-api` submodule by the
...
//! `package ondewo.nlu;` is reachable as `ondewo::nlu::<Message>` and each gRPC service
//! as `ondewo::nlu::<service>_client::<Service>Client`.
//!
