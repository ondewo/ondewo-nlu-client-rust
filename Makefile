...
update_cargo_version: ## Update Version in Cargo.toml
	...
	@echo "$(GREEN)[SUCCESS]$(NC) Cargo.toml version set to ${ONDEWO_NLU_VERSION}"
...
# run (no secret yet, GitHub Actions unavailable, or a re-publish after a fixed workflow).
	@echo "$(BLUE)[INFO]$(NC) Tag ${ONDEWO_NLU_VERSION} pushed - .github/workflows/release.yml publishes it to crates.io"
...
# ($$VAR), not through make ($(VAR)), so the token is not interpolated into the recipe text either.
	...
	@echo "$(BLUE)[INFO]$(NC) Publishing ondewo-nlu-client ${ONDEWO_NLU_VERSION} to crates.io ..."
	cargo publish
